...
exports.shrinkEndingItems = shrinkEndingItems;
...
function shrinkEndingItems(dimensionItems, items, keys, canvasTimeStart, canvasTimeEnd, canvasWidth, shrinkFraction, draggingItem, resizingItem) {
  if (!dimensionItems || dimensionItems.length === 0) {
    return;
  }
  var itemsById = {};
  items.forEach(function (it) {
    itemsById[(0, _generic._get)(it, keys.itemIdKey)] = it;
  });
  dimensionItems.forEach(function (di) {
    var item = itemsById[di.id];
    ...
    var itemEnd = (0, _generic._get)(item, keys.itemTimeEndKey);
    var groupId = (0, _generic._get)(item, keys.itemGroupKey);
    var cellStart = (0, _moment["default"])(itemEnd).startOf('day').valueOf();
    var cellEnd = cellStart + 24 * 60 * 60 * 1000;
    if (!(itemEnd > cellStart && itemEnd < cellEnd)) {
      return;
    }
    var trailingFree = !items.some(function (other) {
      if (other === item) return false;
      if ((0, _generic._get)(other, keys.itemGroupKey) !== groupId) return false;
      var os = (0, _generic._get)(other, keys.itemTimeStartKey);
      var oe = (0, _generic._get)(other, keys.itemTimeEndKey);
      return os < cellEnd && oe > itemEnd;
    });
    if (!trailingFree) {
      return;
    }
    var cellLeftPx = calculateXPositionForTime(canvasTimeStart, canvasTimeEnd, canvasWidth, cellStart);
    var cellWidthPx = calculateXPositionForTime(canvasTimeStart, canvasTimeEnd, canvasWidth, cellEnd) - cellLeftPx;
    var targetRight = cellLeftPx + cellWidthPx * shrinkFraction;
    var newWidth = targetRight - di.dimensions.left;
    if (newWidth >= 3 && newWidth < di.dimensions.width) {
      di.dimensions.width = newWidth;
    }
  });
}
function stackTimelineItems(items, groups, canvasWidth, canvasTimeStart, canvasTimeEnd, keys, lineHeight, itemHeightRatio, stackItems, draggingItem, resizingItem, dragTime, resizingEdge, resizeTime, newGroupOrder, endingItemShrinkFraction) {
  ...
  if (endingItemShrinkFraction != null) {
    shrinkEndingItems(dimensionItems, visibleItemsWithInteraction, keys, canvasTimeStart, canvasTimeEnd, canvasWidth, endingItemShrinkFraction, draggingItem, resizingItem);
  }
